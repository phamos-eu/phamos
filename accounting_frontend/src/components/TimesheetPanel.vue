		...
		<div class="tp__row">
			<template v-if="!activeSession">
				<span class="tp__label" :title="projectLabel">
					{{ projectLabel || "No Accounting project configured" }}
				</span>
				<button
					type="button"
...
const TIMER_API = "phamos.phamos.page.dev_action_panel.dev_action_panel"
const SETTINGS_API = "phamos.api.accounting_spa"
...
async function loadSettings() {
	const settings = await call(`${SETTINGS_API}.get_accounting_settings`)
	timesheetProject.value = settings.accounting_standard_project
	projectLabel.value = settings.accounting_standard_project_name || settings.accounting_standard_project || ""
}
...
async function onStartConfirm({ goal, expectedTime, manualStartTime, task }) {
	...
	if (!timesheetProject.value) {
		toast.error("Configure Accounting Standard Project in phamos Settings.")
		return
	}
	try {
		const args = {
