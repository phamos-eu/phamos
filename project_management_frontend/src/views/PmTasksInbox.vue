		...
		>
			...
			<template v-else-if="!filteredTasks.length && layout !== 'gantt'">
				<div class="flex flex-1 flex-col items-center justify-center gap-2 px-6 text-center">
					<p class="font-medium text-gray-900 dark:text-gray-100">No Project Management tasks found</p>
					<p class="max-w-sm text-sm text-gray-500 dark:text-gray-400">
						Tasks linked to the Project Management department configured in phamos Settings will appear here.
					</p>
				</div>
			</template>
			<TaskGantt
...
async function loadSettings() {
	try {
		const settings = await call(`${API}.get_pm_settings`)
		chatFlags.value = settings.chat || chatFlags.value
		configError.value = settings.pm_department ? "" : "Configure Project Management Department in phamos Settings."
	} catch (e) {
		configError.value = e?.messages?.[0] || e?.message || "Could not load Project Management settings"
	}
}
