...
def get_context(context):
    ...
    args = req.args if req else frappe._dict()
    selected_types = args.getlist("event_type") if hasattr(args, "getlist") else []
    selected_city  = args.get("city", "")
    show_past      = args.get("show_past", "")
    ...
    events = frappe.get_all(
        ...
        fields=[
            ...
            "starts_on", "ends_on", "location", "city",
            "image", "intro", "route",
    ...
    for e in events:
        if e.starts_on:
            dt = get_datetime(e.starts_on)
            e.starts_date = dt.strftime("%d %b %Y")   # "25 Mar 2026"
            e.starts_time = dt.strftime("%H:%M")       # "08:37"
        else:
            e.starts_date = ""
            e.starts_time = ""
