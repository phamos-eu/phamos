...
async function loadSettings() {
	const settings = await call(`${SETTINGS_API}.get_hr_settings`)
	timesheetProject.value = settings.hr_timesheet_project
	projectLabel.value = settings.hr_timesheet_project_name || settings.hr_timesheet_project || ""
}
...
async function onStartConfirm({ goal, expectedTime, manualStartTime, task }) {
	...
	if (!timesheetProject.value) {
		toast.error("Configure HR Timesheet Project in phamos Settings.")
		return
	}
	try {
		const args = {
