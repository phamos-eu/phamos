frappe.provide("phamos.functional_calendar");

(function () {
	const SCHEDULE_TABLE_FIELDS = [
		"custom_team_daily_schedule",
		"appointment_schedule",
		"custom_appointment_schedule",
	];

	function resolve_table_field(frm) {
		if (!frm || !frm.fields_dict) {
			return null;
		}
		for (const fieldname of SCHEDULE_TABLE_FIELDS) {
	...
	function setup_email_account_query(frm, table_field) {
		const fieldname = table_field || resolve_table_field(frm);
		if (!fieldname) {
			return;
		}
		const grid = frm.fields_dict[fieldname] && frm.fields_dict[fieldname].grid;
		if (!grid || !grid.get_field("email_account")) {
			return;
		}
		...
		grid.get_field("email_account").get_query = function () {
			return {
				query:
					"phamos.mailcow_integration.doctype.functional_calendar_entry.functional_calendar_entry.email_account_with_dav_password_query",
			};
		};
	}

	function show_free_slots_dialog(frm, row, cdt, cdn) {
		if (!row.email_account) {
			frappe.msgprint(__("Please select Email Account first. It is required for fetching slots."));
			return;
		}

		const table_field = resolve_table_field(frm);
		...
		const render_slots = (slots) => {
			...
			slots.forEach((slot, i) => {
				const label =
					slot.label || `${slot.start_local || slot.start} -> ${slot.end_local || slot.end}`;
			...
			if (slots.length > 3) {
				wrap.append(
					`<div class="text-muted small mb-1">${__("Scroll horizontally to see more slots")}</div>`
				);
			}
			...
			const setSelected = (idx) => {
				...
				wrap
					.find(`.nav-link[data-bs-target="${selPaneId}"], .nav-link[data-target="${selPaneId}"]`)
			...
			slots.forEach((slot, i) => {
				...
				wrap.find(`#${tabId} button`).on("click", () => {
					...
					setSelected(i);
					frappe.show_alert({
						message: __("Selected: {0} -> {1}", [startUse, endUse]),
						indicator: "green",
					});
				});
			});
			...
			wrap.on(
				"shown.bs.tab",
				`.nav-link[data-target^="#${tabsId}-tab-"], .nav-link[data-bs-target^="#${tabsId}-tab-"]`,
				function () {
	...
	phamos.functional_calendar = {
		SCHEDULE_TABLE_FIELDS,
		resolve_table_field,
		setup_email_account_query,
		show_free_slots_dialog,
	};
})();
