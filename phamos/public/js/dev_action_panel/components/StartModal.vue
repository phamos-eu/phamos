      ...
      <div class="mo__header">
        ...
        <div>
          ...
          <p class="mo__subtitle">
            <a v-if="issue.issue_url" :href="issue.issue_url" target="_blank" class="mo__issue-link">
              #{{ issue.issue_id }}
            </a>
            <span v-else>#{{ issue.issue_id }}</span>
            &nbsp;{{ issue.title }}
          </p>
        </div>
...
.mo__issue-link { color: var(--primary); text-decoration: none; font-weight: 600; }
.mo__issue-link:hover { text-decoration: underline; }
.mo__close {
  margin-left: auto;
...
.mo__textarea:focus, .mo__input:focus {
  outline: none;
