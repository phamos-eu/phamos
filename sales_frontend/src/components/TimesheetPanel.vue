		...
		<div class="tp__row">
			<template v-if="!activeSession">
				<span class="tp__label" :title="projectLabel">
					{{ projectLabel || "No Sales project configured" }}
				</span>
				<button
					type="button"
...
const SETTINGS_API = "phamos.api.sales_spa"
...
async function loadSettings() {
	const settings = await call(`${SETTINGS_API}.get_sales_settings`)
	timesheetProject.value = settings.sales_standard_project
	projectLabel.value = settings.sales_standard_project_name || settings.sales_standard_project || ""
}
...
async function onStartConfirm({ goal, expectedTime, manualStartTime, task }) {
	...
	if (!timesheetProject.value) {
		toast.error("Configure Sales Standard Project in phamos Settings.")
		return
	}
	try {
		const args = {
