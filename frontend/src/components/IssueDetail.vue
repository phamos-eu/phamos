<template>
	<div class="flex h-full min-h-0 flex-col overflow-y-auto p-5">
		<header class="mb-4 flex items-start justify-between gap-3">
			<div>
				<div class="text-xs font-semibold text-gray-500">{{ issue.name }}</div>
		...
		<section class="mb-5">
			...
			<div
				...
				class="prose prose-sm max-w-none text-sm text-gray-800 [&_img]:max-w-full [&_img]:rounded-md"
				v-html="issue.description"
			></div>
			<div v-else class="text-sm text-gray-500">No description</div>
		</section>
		...
		<section class="mb-5">
			...
			<div class="mb-3 max-h-48 overflow-y-auto rounded-md border border-gray-200 p-1">
				<label
...
async function changeStatus(next) {
	...
	try {
		const updated = await call(`${API}.update_status`, {
...
async function saveAssignees() {
	...
	try {
		const updated = await call(`${API}.set_assignees`, {
			name: props.issue.name,
