...
def get_financial_history(name, customer):
	...
	if len(get_project_list) == 1:
		...
		timesheet_hrs = frappe.db.sql("""SELECT sum(td.hours) as timesheet_hrs from `tabTimesheet` t join `tabTimesheet Detail` td on t.name = td.parent where td.is_billable = 1 and t.docstatus = 0 and td.project in {0} and td.custom_implementation = '{1}'  """.format(tuple(get_project_list), name), as_list=1, debug=1)
