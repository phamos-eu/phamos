...
"""Parent-agnostic Mailcow sync for Functional Calendar Entry child tables.

Register parent doctypes and their Table fieldnames in SCHEDULE_PARENTS, then
hook ``sync_events_from_parent`` / ``cleanup_events_on_parent_trash`` on those
doctypes in hooks.py.
"""
...
# Parent DocType -> child Table fieldname that uses Functional Calendar Entry
SCHEDULE_PARENTS: dict[str, str] = {
...
SCHEDULE_ROW_DOCTYPE = "Functional Calendar Entry"
AUTO_GENERATED_MARKER = "***** Auto Generated *****"
LEGACY_UID_PREFIX = "phamos-team-daily-"


def get_schedule_table_field(doctype: str) -> str | None:
	return SCHEDULE_PARENTS.get(doctype)
...
def _row_uid(parent_doctype: str, row_name: str) -> str:
	safe_dt = (parent_doctype or "doc").lower().replace(" ", "-")
	return f"phamos-sched-{safe_dt}-{row_name}"


def _legacy_team_uid(row_name: str) -> str:
	return f"{LEGACY_UID_PREFIX}{row_name}"


def _resolve_row_uid(doc, row) -> str:
	existing = (row.get("mailcow_uid") or "").strip()
	if existing:
		return existing
	# Preserve historical Team UIDs so re-sync does not duplicate SOGo events
	if doc.doctype == "Team":
		return _legacy_team_uid(row.name)
	return _row_uid(doc.doctype, row.name)
...
def _schedule_field(doc) -> str | None:
	fieldname = get_schedule_table_field(doc.doctype)
	if not fieldname:
		return None
...
def _cleanup_removed_row_events(doc, table_field: str):
	previous_doc = doc.get_doc_before_save()
	if not previous_doc:
		return

	previous_rows = {row.name for row in (previous_doc.get(table_field) or []) if row.name}
	current_rows = {row.name for row in (doc.get(table_field) or []) if row.name}
	previous_rows_by_name = {
		row.name: row for row in (previous_doc.get(table_field) or []) if row.name
	}
	...
	for removed_row_name in previous_rows - current_rows:
		try:
			...
			if mailbox_email:
				uid = (removed_row.get("mailcow_uid") or "").strip() or (
					_legacy_team_uid(removed_row_name)
					if doc.doctype == "Team"
					else _row_uid(doc.doctype, removed_row_name)
				)
				ok, error = _delete_uid_for_mailbox(uid, mailbox_email)
				if not ok:
					frappe.log_error(
						error or "Unknown delete error",
						"Functional Calendar Entry Mailcow DELETE",
					)
		except Exception:
			frappe.log_error(frappe.get_traceback(), "Functional Calendar Entry Mailcow DELETE")


def sync_events_from_parent(doc, method=None):
	"""Sync Functional Calendar Entry rows on a registered parent to Mailcow."""
	table_field = _schedule_field(doc)
	if not table_field:
		return

	_cleanup_removed_row_events(doc, table_field)

	previous_doc = doc.get_doc_before_save()
	previous_rows = (previous_doc.get(table_field) or []) if previous_doc else []
	previous_rows_by_name = {row.name: row for row in previous_rows if row.name}
		...
		try:
			...
			if previous_mailbox and mailbox_email and previous_mailbox.lower() != mailbox_email.lower():
				...
				if not ok:
					_mark_row_error(
						row.name,
						error or "Unknown mailbox move delete error",
						mailbox_email=previous_mailbox,
					)
					continue
			...
			frappe.log_error(frappe.get_traceback(), "Functional Calendar Entry Mailcow Sync")
