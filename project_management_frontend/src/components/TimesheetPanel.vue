		...
		<div class="tp__row">
			<template v-if="!activeSession">
				<span class="tp__label" :title="projectLabel">
					{{ projectLabel || "No Project Management project configured" }}
				</span>
				<button
					type="button"
...
const SETTINGS_API = "phamos.api.project_management_spa"
...
async function loadSettings() {
	const settings = await call(`${SETTINGS_API}.get_pm_settings`)
	timesheetProject.value = settings.pm_timesheet_project
	projectLabel.value = settings.pm_timesheet_project_name || settings.pm_timesheet_project || ""
}
...
async function onStartConfirm({ goal, expectedTime, manualStartTime, task }) {
	...
	if (!timesheetProject.value) {
		toast.error("Configure Project Management Timesheet Project in phamos Settings.")
		return
	}
	try {
		const args = {
