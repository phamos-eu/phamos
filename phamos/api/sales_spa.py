...
def check_app_permission():
	...
	if roles & {"System Manager", "Sales Manager", "Sales User"}:
		return True
...
def _get_sales_settings_doc():
	return frappe.get_cached_doc("phamos Settings")


def _get_sales_department():
	return frappe.db.get_single_value("phamos Settings", "sales_department")


def _get_sales_standard_project():
	return frappe.db.get_single_value("phamos Settings", "sales_standard_project")


def _get_sales_project_names(department=None):
	"""Projects linked to the Sales department."""
	department = department or _get_sales_department()
	if not department:
		return []
...
def _sales_issue_or_filters(department=None):
	"""Match issues scoped to Sales via project department or Issue.custom_department."""
	department = department or _get_sales_department()
	if not department:
		return []

	or_filters = []
	project_names = _get_sales_project_names(department)
	if project_names:
...
def _issue_in_sales_scope(doc):
	"""True when issue belongs to the configured Sales department."""
	department = _get_sales_department()
	if not department:
		return False
...
def _ensure_issue_in_sales_scope(doc):
	if not _issue_in_sales_scope(doc):
		frappe.throw(_("This issue is not in the Sales department scope."))


def _require_sales_department():
	department = _get_sales_department()
	if not department:
		frappe.throw(
			_("Sales Department is not configured in phamos Settings."),
			title=_("Configuration required"),
		)
	return department


def _validate_sales_project(project):
	if not project:
		frappe.throw(_("Project is required for Sales issues."))
	department = _require_sales_department()
	project_department = frappe.db.get_value("Project", project, "department")
	if project_department != department:
		frappe.throw(
			_("Project {0} is not linked to Sales department {1}.").format(project, department)
		)
...
def get_sales_settings():
	...
	department = _get_sales_department()
	standard_project = _get_sales_standard_project()
	project_names = _get_sales_project_names(department)
	...
	return {
		...
		"sales_standard_project": standard_project,
		"sales_standard_project_name": project_label,
		"sales_project_count": len(project_names),
...
def get_timesheet_project_tasks(project=None):
	...
	department = _require_sales_department()
	project = project or _get_sales_standard_project()
	if not project:
		return []

	_validate_sales_project(project)
...
def get_issue(name):
	...
	_ensure_issue_in_sales_scope(doc)
...
def get_form_options():
	...
	project_names = set(_get_sales_project_names(department))
	standard_project = _get_sales_standard_project()
	if standard_project:
	...
	options["sales_department"] = department
	options["sales_standard_project"] = _get_sales_standard_project()
...
def create_issue(
	...
	sales_department = _require_sales_department()

	if not project:
		project = _get_sales_standard_project()
	if not department:
		department = sales_department

	if department != sales_department and project:
		_validate_sales_project(project)
...
def get_task(name):
	...
	if doc.department != department:
		frappe.throw(_("This task is not in the Sales department."))
...
def create_task(
	...
	if department != sales_department:
		frappe.throw(_("Task department must match Sales department in phamos Settings."))
