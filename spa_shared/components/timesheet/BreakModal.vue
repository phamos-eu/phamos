...
onMounted(async () => {
  ...
  try {
    const options = await call(`${spaConfig.api}.get_form_options`);
    allowedProjects.value = options?.projects || [];
  } catch {
...
async function fetchProjects(term) {
  ...
  let rows = allowedProjects.value || [];
  if (needle) {
    rows = rows.filter((p) => {
      const label = `${p.project_name || ""} ${p.name || ""}`.toLowerCase();
      return label.includes(needle);
    });
  }
  projectResults.value = rows.slice(0, 50);
}
