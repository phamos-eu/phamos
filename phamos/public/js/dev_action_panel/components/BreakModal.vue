...
const breakDuration = computed(() => {
  ...
  if (sysTz && typeof moment !== "undefined" && moment.tz) {
    try { secs = Math.max(0, moment().diff(moment.tz(props.breakFrom, sysTz), "seconds")); } catch(e) {}
  } else {
    const from = new Date(props.breakFrom.replace(" ", "T"));
...
function submit() {
  if (!project.value)        { frappe.msgprint(__("Please select a project.")); return; }
  ...
  if (!result.value.trim())  { frappe.msgprint(__("Please describe what you did during the break.")); return; }
  emit("confirm", {
